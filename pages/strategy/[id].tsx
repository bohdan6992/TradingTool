...
export default function StrategyPage() {
  ...
  return (
    <div className="min-h-screen selection:bg-cyan-400/20">
      ...
      {/* STATS STRIP — тримаємо 7 колонок у масштабі */}
      <section
        className="scale-lock max-w-7xl mx-auto px-5 mt-6 tt-grid sm:grid-cols-2 xl:grid-cols-7 gap-5" // ⬅️ scale-lock
        data-cols="7"
      >
      ...
      {/* CHARTS — одиночний (може лишатися без замка) */}
      ...
      {/* CHARTS — ряд із 3 картками, не перебудовуємо */}
      <section
        className="scale-lock max-w-7xl mx-auto px-5 mt-6 tt-grid lg:grid-cols-3 gap-6" // ⬅️ scale-lock
        data-cols="3"
      >
      ...
      <section className="max-w-7xl mx-auto px-5 mt-6">
        ...
        <style jsx>{`
          .topGrid {
            display: grid;
            grid-template-columns: 1fr 1fr;  /* завжди дві колонки */
            gap: 20px;
            align-items: stretch;
          }

          /* прибрано @media — тепер грід не "з'їжджає" */
          .subcard {
            ...
            height: 100%;
            min-width: 0; /* не розтягує колонку */
          }
      ...
      <section className="max-w-7xl mx-auto px-5 mt-6 mb-14">
                  ...
                  <div className="main">
                    ...
                    <div className="meta">
                      ...
                      <span className="entry">
                        Вхід: <b>{t.entry_amount == null ? "—" : fmt(Number(t.entry_amount))}</b>
                      </span>
                    </div>
                  </div>
                  <div className="right">
        ...
        <style jsx>{`
          .rlist { display: flex; flex-direction: column; gap: 10px; padding: 10px; }
          .row {
            display: grid;
          ...
          .pill {
            font-weight: 800; letter-spacing: .3px; padding: 6px 10px; border-radius: 999px;
          ...
          .meta { display: flex; align-items: center; gap: 8px; opacity: .8; font-size: .92rem; min-width: 0; color: var(--fg); }
          .date { font-variant-numeric: tabular-nums; }
          .sep  { opacity: .6; }
          .right { display: flex; align-items: center; gap: 10px; }
          .res {
            padding: 6px 12px; border-radius: 12px; font-weight: 800;
            font-variant-numeric: tabular-nums; color: #fff; min-width: 92px; text-align: right;
          ...
          .btnGhost, .btnDanger {
            height: 34px; width: 34px; padding: 0; display: inline-flex; align-items: center; justify-content: center;
            border-radius: 10px; border: 1px solid var(--card-ring);
            background: color-mix(in oklab, var(--card-bg) 85%, transparent);
            color: var(--fg); transition: all .2s ease; cursor: pointer; text-decoration: none; font-size: 16px;
          }
          .btnGhost:hover { border-color: color-mix(in oklab, var(--color-primary) 35%, var(--card-ring)); box-shadow: 0 6px 16px rgba(0,0,0,.18); }
          .btnDanger {
            color: #fff;
            background: linear-gradient(
              180deg,
          ...
          .btnDanger:hover { transform: translateY(-1px); box-shadow: 0 8px 18px rgba(0,0,0,.2); }
...
function StatTile({
  ...
  return (
    <motion.div
      ...
      style={{ boxShadow: "0 6px 24px rgba(0,0,0,.15)" }}
    >
      ...
      {sub ? (
        <div className="mt-2" style={{ fontSize: 12, opacity: 0.75 }}>
          {sub}
        </div>
      ) : null}
    </motion.div>
